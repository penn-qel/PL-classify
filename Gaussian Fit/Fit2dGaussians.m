function [FitResult,BestFit,Errs,bfpars] = Fit2dGaussians(data,fitopts)
% Function to fit 2d data to a set of (symmetric) Gaussian peaks
%
...
% 'StartWidths' (scalar or Nx1 vector): initial width for each Gaussian
...
% 'fittypes' (Nx3 logical array): Needed if 'mode'=='Arbitrary' to specify
%   freedom of each parameter, with rows giving [posn,width,ht] and values
%   0==fixed, 1==free
...
parse_fitopts.FunctionName = 'Fit2dGaussians';
...
addParameter(parse_fitopts,'mode',...
    ...
    @(x) any(strcmp(x,{'AllFree','FixedPosition','FixedWidth','FixedHeight','Arbitrary'})));
addParameter(parse_fitopts,'fittypes',...
    true(np,3),... % default is all free (if not given)
    @(x) validateattributes(x,{'logical','numeric'},{'binary','size',[np,3]}));
...
if length(fitpars.StartWidths) == 1
    fitpars.StartWidths = repmat(fitpars.StartWidths,np,1);
end
...
switch fitpars.mode
    case 'AllFree'
        fitpars.fittypes = true(np,3); % override any input values and set all parameters to be free
    case 'FixedPosition'
...
lim_widths = fitpars.LimWidths(fitpars.fittypes(:,2),:);
...
npars = length(freeposns)+length(freewidths)+length(freehts)+length(freedrift)+length(freebg);
...
bfpars = Create2dGaussianModel(FitResult.x,fitpars);
...
if nargout>2 && FitResult.exitflag>0 % only calculate if requested and fit was successful
    ...
    err_struct = Create2dGaussianModel(FitResult.sigma_x,fitpars); % will collate signa_x into structure (values of fixed parameters are starting positions, but get replaced by NaN below)
...
model = Create2dGaussianModel(x,fitoptions);
...
function model = Create2dGaussianModel(x,fitoptions)

fittypes = fitoptions.fittypes;

npars = sum(fittypes(:)) + sum(fittypes(:,1)) + 2*fitoptions.CommonDrift + ~fitoptions.FixedBkgnd; % expected number of free parameters
if length(x)~=npars
...
allpars = NaN(fitoptions.npeaks,4); % will hold all parameters in array giving [x,y,w,a] as columns
% Parse positions
...
if any(freew)
    if fitoptions.CommonWidth
        ...
        allpars(:,3) = x(lastix+1); % all widths are set to the same value
        lastix = lastix+nfree; % should be lastix+1
    else
        allpars(freew,3) = x(lastix+(1:nfree));
        lastix = lastix + nfree;
        allpars(~freew,3) = fitoptions.StartWidths(~freew);
    end
else
    allpars(:,3) = fitoptions.StartWidths;
end
% Parse amplitudes  
freeht = fittypes(:,3);
nfree = sum(freeht);
if any(freeht)
    allpars(freeht,4) = x(lastix+(1:nfree));
    lastix = lastix+nfree;
    allpars(~freeht,4) = fitoptions.StartHts(~freeht);
else
    allpars(:,4) = fitoptions.StartHts;
end
...
%   'w': Nx1 vector of Gaussian standard deviation (symmetric in x,y)
...
for pp=1:np
    ...
    if isfinite(cutoff)
        PtsInRange = (dist2 <= (cutoff*w)^2); % points located within cutoff*w of (xp,yp)
    else
        PtsInRange = true(size(X));
    end
    ThisPeak = zeros(size(X));
    ThisPeak(PtsInRange) = a*exp(-dist2(PtsInRange)/(2*w^2));
    Z = Z+ThisPeak;
end
